                            ...
                            ...) 
{
    ...
    #xavi: baseProbVec <- 1/(1 + rowSums(probMat, na.rm = TRUE))   # P_i0  #xavi: to avoid NA as results when NA present in data frame
...
## xavi: here start changes in order to include (environmental) constraints of the choice (crops in our case) 
    ...
    switch(type, class_constr={
      ...
      Y <- as.data.frame(probMat)
      choice <- apply(Y, 1, function(x) object$choices[which(x == max(x, na.rm = TRUE))])
